...
<br><br>
	<center>
					<nav id="nav" style="width: 100%">
						<ul>
							<br>
<%-- 				<c:forEach var="chat2" items="${list}">
							<li><a href="#${chat2.chatnum}" class="active">${chat2.chatnum}Day</a></li>
				</c:forEach> --%>
						</ul>
					</nav>
<div id="main">
	...
	<c:if test="${empty sessionScope.myname}">
	...
	<c:if test="${not empty sessionScope.myname}">
						...
						data-re_level="${chat.re_level}" data-b_num=${chat.b_num }>		<!-- 답글 쓰기 -->
							<form action="replyInsert.bd">
								<input type="hidden" name="b_num" value="${bb.num}"> <input
									type="hidden" name="writer" value="${sessionScope.myname}"> <input
